...
- [x] 6.3 Extend Go RPC models/client methods and XML-RPC compatibility fixtures for nullable persistent fields, pagination, and both log response types.
- [x] 6.4 Add required Run-name and optional-purpose controls to the Wails start form and display number, metadata, email status, and delivery errors in history/detail views.
- [x] 6.5 Add incremental Run/task log views with bounded polling, task/attempt filters, empty/loading/error states, and no log payload in the existing Run-list polling path.
